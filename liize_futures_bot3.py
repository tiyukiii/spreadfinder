...
bot = telebot.TeleBot("7519687252:AAEOepjfLrJIC4dcZ4iya5wNnuZMfdiQFOc")
    ...
    try:
        ...
        for pair in pairs:
            ...
            if liquidity:
                ...
                if liquidity_usd and liquidity_usd < 2:
...
def get_mexc_price(symbol):
    try:
        ...
        # Example match: BTC_USDT, ETH_USDT, etc.
        for item in data:
            if item["symbol"] == f"{symbol}_USDT":
                return float(item["lastPrice"])
        return None
    except Exception as e:
        print(f"❌ Ошибка получения фьючерсной цены MEXC для {symbol}: {e}")
        return None
